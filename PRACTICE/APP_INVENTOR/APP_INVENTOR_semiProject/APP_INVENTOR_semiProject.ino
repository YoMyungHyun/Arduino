...
float avgDustDensity = 0; //"" 평균

void setup() {
  dht.begin(); //온.습도 센서 동기화 
  mySerial.begin(9600); //소프트웨어 시리얼 동기화
  pinMode(OUTPUT_VOLTAGE, INPUT); //A0 핀 입력 설정
  pinMode(DC_MOTOR, OUTPUT); //6번 핀 출력 설정
  pinMode(PULSE, OUTPUT); //7번 핀 출력 설정
  pinMode(ROOM_LED, OUTPUT); //12번 핀 출력 설정
  pinMode(LIVINGROOM_LED, OUTPUT); //13번 핀 출력 설정
}
...
void loop() { 
  ...
  if (isnan(humidity) || isnan(temperature)) { //데이터를 읽어오지 못하면 함수 종료
    return;
  }
  
  sumDustDensity = 0; //미세먼지 농도 수치 합 초기화 
  for(int i=0;i<30;i++) //for문으로 미세먼지 농도 수치 30회 측정
  {
...
void motor_Control(byte dcStep) //motor_Control 함수 정의
{
  switch(dcStep) //현재 DC 모터 속도
  {
    ...
    case 1: 
      ...
      break;
    case 2: 
      analogWrite(DC_MOTOR, 64); //1/4 속도로 DC 모터 출력
      break;
    case 3: 
