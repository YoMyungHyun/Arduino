...
#define GREEN_PIN 12 //12번 GREEN 핀 제어
#define RED_PIN 13 //13번 RED 핀 제어

bool occupied = false; //화장실 사용 여부를 판단하는 변수 선언

void setup() {
  // put your setup code here, to run once:
  pinMode(MAGNETIC, INPUT_PULLUP); //7번 핀을 INPUT_PULLUP 설정
  pinMode(PIR, INPUT); //8번 핀은 입력 설정
  pinMode(GREEN_PIN, OUTPUT); //12번 핀을 출력 설정
  pinMode(RED_PIN, OUTPUT); //13번 핀을 출력 설정
...
void loop() {
  ...
  {
    occupied = true; //화장실을 사용하고 있다는 의미로 true
  }
  else if(magneticValue == 1 && pirValue == 1) //화장실 문이 열려있고 동작이 감지되었을 때(화장실 내 사람이 없을 때)
  {   
       occupied = false; //화장실을 사용하고 있지 않다는 의미로 false    
  }
