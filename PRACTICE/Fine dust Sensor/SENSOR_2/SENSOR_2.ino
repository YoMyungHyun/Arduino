...
void setup() {                
  ...
  pinMode(RED_PIN, OUTPUT);  //11번 핀 출력 설정
  pinMode(GREEN_PIN, OUTPUT);   //10번 핀 출력 설정
  pinMode(BLUE_PIN, OUTPUT);  //9번 핀 출력 설정
  Serial.begin(9600); 
} 
...
void loop() {
  ...
  if(avgDustDensity >= 101){    //매우 나쁨
    colorPrint(255, 0, 0);  //빨간색 점등
  } else if(avgDustDensity >= 51){    //나쁨
    colorPrint(255, 10, 0);   //주황색 점등
  } else if(avgDustDensity >= 31){    //보통
    colorPrint(0, 255, 0);   //주황색 점등
  } else{   //좋음
...
void colorPrint(int red, int green, int blue){    //3색 LED 출력 함수
