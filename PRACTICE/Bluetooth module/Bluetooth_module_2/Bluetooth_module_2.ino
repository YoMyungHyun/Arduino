...
#define DHTPIN 7    //온.습도 센서 out 7번 핀으로 설정
#define DHTTYPE DHT11   //온.습도 센서 타입 선택
#define OUTPUT_VOLTAGE A0   //A0 핀으로 출력 전압 입력

SoftwareSerial mySerial(TXD, RXD);    //소프트웨어로 시리얼 mySerial 객체 선언
DHT dht(DHTPIN, DHTTYPE);   //초기화
...
void setup() {
  ...
  dht.begin();    
  pinMode(INPUT_PULSE, OUTPUT);   //4번 핀 출력
  pinMode(OUTPUT_VOLTAGE, INPUT);   //A0 핀 입력
}

void loop() {
  delay(2000);

  float humidity = dht.readHumidity();    //습도 데이터를 읽어 변수에 저장
  float temperature = dht.readTemperature();    //온도 데이터를 읽어 변수에 저장
  sumDustDensity = 0;   //미세먼지 농도 수치 합 초기화
  for(int i = 0; i < 30; i++){    //미세먼지 농도 수치 30회 측정
    digitalWrite(INPUT_PULSE, LOW);   //인가
    delayMicroseconds(280);   //0.23ms 대기
    preVoltage = analogRead(OUTPUT_VOLTAGE);
    delayMicroseconds(40);
    digitalWrite(INPUT_PULSE, HIGH);    //종료
